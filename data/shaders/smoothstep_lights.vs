...
vec4 useLight(Light light, vec4 surfacePostion, vec4 normal_transformed, vec3 normal, Material material )
{
    ...
    if(light.light_position.w == 1.0)
    {
        ...
        // reduce the attenuation to 0.0 using the smoothstep function

		attenuation = 1.0 - smoothstep(light.cone_angle,  light.cone_angle + smoothstep_delta , light_to_surface_angle);
