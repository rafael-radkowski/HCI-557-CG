...
inline void CreateRenderToTexture(int texture_width, int texture_height, unsigned int& frame_buffer_object, unsigned int& texture_color,  unsigned int& texture_depth)
{
	...
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_DEPTH_COMPONENT32, texture_width, texture_height);
...
@param texture_depth -  a variable in which this function can write the depth texture idx into.
*/
inline void CreateRenderToTexture32Bit(int texture_width, int texture_height, unsigned int& frame_buffer_object, unsigned int& texture_color, unsigned int& texture_depth)
{
	// Create a frame buffer
	//glCreateFramebuffers(1, &frame_buffer_object);
	glGenFramebuffers(1, &frame_buffer_object);
	...
	glGenTextures(1, &texture_color);
	glBindTexture(GL_TEXTURE_2D, texture_color);
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA32F_ARB, texture_width, texture_height);
